...
def main(f=None):
    ...
    for _ in range(1, 11):
        ...
        while cque:
            ...
            for dir in range(4):
                Bnxt = rBnxt = Bcur + mov[dir][Bcur]
                Rnxt = rRnxt = Rcur + mov[dir][Rcur]

                # 구슬 위치가 겹치는 경우
                if rBnxt == rRnxt:
                    rRnxt += bak[dir]
                    if check(dir, Bcur, Rcur):
                        rRnxt, rBnxt = rBnxt, rRnxt

                # 방문한 위치에 있는 경우
                if rBnxt * 100 + rRnxt in vis:
                    continue
                ...
                # 다음 큐에 추가
                vis.add(rBnxt * 100 + rRnxt)
                nque.append(rBnxt * 100 + rRnxt)
